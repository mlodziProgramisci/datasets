...
const kinds = [];
function addKind(name, group, colors, description, typicalActivityLevel, typicalAggressiveness, size, minHealthyMass, maxHealthyMass, minTypicalLifespan, maxTypicalLifespan) {
...
Jamnik wymaga szczególnej troski i zainteresowania właściciela. Psy te mają szczególne wymagania jeśli chodzi o tresurę i rozrywkę. Wymagają dużo uwagi, w przeciwnym razie mogą stać się agresywne i groźne. Niektóre nie tolerują dzieci. Przy odpowiednim ułożeniu jamnik może bardzo zaprzyjaźnić się z dzieckiem.`, "average", "safe for adults", "small", 3.5, 14.5, 12, 16);
addKind('Rottweiler', 'Pinczery i sznaucery, molosy, szwajcarskie psy górskie i do bydła, pozostałe rasy', ['czarny', 'brązowy'], `Rottweiler z natury jest psem obronnym i agresywnym wobec intruzów. Według wzorca jest to pies o zrównoważonej psychice, wymagający odpowiedniego i konsekwentnego podejścia. Pewność siebie jest główną cechą charakteru tej rasy, podobnie jak upór, samodzielność czy skłonność do dominacji (ujawnia się to szczególnie u samców). W rękach osoby brutalnej i niezrównoważonej rottweiler staje się nieprzewidywalny i może stanowić zagrożenie dla otoczenia, z właścicielem włącznie. Pies ten potrzebuje sporej dawki codziennego ruchu: regularnych treningów i szkoleń oraz aktywnych spacerów.`, "hyperactive", "dangerous", "big", 43, 63, 11, 13);
addKind('Mops', 'Psy ozdobne i do towarzystwa', ["czarny", "płowy"], `Pies tej rasy jest kontaktowy i towarzyski, więc źle znosi długotrwałą samotność. Wykazuje niewielki stopień posłuszeństwa. Szybko przystosowuje się do życia w rodzinie.
Ze względu na obciążenie chorobami, moralność chodowli tej rasy poddawana jest w wątpliwość.`, "lazy", "safe for kids", "small", 6.3, 8.1, 12, 15);
// addKind('Jack Russell terrier', 'Teriery'
addKind('Dalmatyńczyk', 'Psy gończe i rasy pokrewne', ["biało-czarne", "biało-brązowe"], `Jest to pies odważny, czujny i zrównoważony. Wykazuje wysoki stopień przywiązania do członków rodziny, dobrze czuje się w towarzystwie dzieci. Aktywny i towarzyski, sprawdza się jako pies rodzinny, jeśli jest odpowiednio prowadzony. Na ogół przyjaźnie nastawiony do innych zwierząt. Dalmatyńczyk nie jest psem odpowiednim dla mało ruchliwych osób. Podatny na szkolenie, chętny do nauki, którą z uwagi na niezależność charakteru tej rasy należy zaczynać jak najwcześniej.`, "average", "safe for adults", "big", 55.9, 61, 14, 17);
...
addKind('Husky Syberyjski', 'Szpice i psy w typie pierwotnym', ["szary", "wilczasty", "sobolowy"], `Współcześnie husky syberyjski jest wykorzystywany, tak jak i dawniej, jako pies zaprzęgowy, także jako pies rodzinny.`, "average", "safe for adults", "big", 16, 27, 12, 14);
const dogs = [];
function addDog(nickname, kindName, color, mass, age) {
...
fs_1.default.writeFileSync('dogs.json', result);
fs_1.default.writeFileSync('dogs.js', "var dogs = JSON.parse('" + result + "');");
